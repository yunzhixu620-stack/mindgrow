...
> 最近核对基线：已发布前后端 `main@e895ca8`、API `10.17.0`；S2.17 于 2026-07-23 完成生产事实校验。
...
1. [x] 2026-07-23 在 Supabase Production 控制台确认 custom SMTP 仍为关闭状态；当前使用内置 best-effort 邮件通道，不能作为生产投递完成证据。
2. [x] 完成 Resend、阿里云邮件推送与 Supabase 内置通道的成本、频控、域名、退信和回滚对比；首发推荐 Resend Free + 自有发信子域名，详见 `docs/s2-18-email-delivery.md`。
3. [x] 注册成功、重发成功和服务端频控错误均触发 60 秒浏览器冷却；刷新后保持且不保存邮箱，损坏/过期本地值不会永久锁定用户。
